...
│  Presentation       │  TLS/SSL 加密, JPEG/PNG 编码, UTF-8 转码      │
├─────────────────────┼──────────────────────────────────────────────┤
│  5. 会话层           │  建立、管理、终止会话                           │
│  Session            │  RPC, NetBIOS, TLS 握手阶段也可视为会话管理      │
├─────────────────────┼──────────────────────────────────────────────┤
│  4. 传输层           │  端到端的可靠（或不可靠）数据传输                 │
│  Transport          │  TCP (可靠), UDP (不可靠)                      │
├─────────────────────┼──────────────────────────────────────────────┤
│  3. 网络层           │  逻辑寻址与路由，决定数据包从哪条路走             │
│  Network            │  IP, ICMP, ARP, 路由器工作在这一层              │
├─────────────────────┼──────────────────────────────────────────────┤
│  2. 数据链路层        │  相邻节点间的帧传输，MAC 寻址                    │
│  Data Link          │  Ethernet, Wi-Fi, 交换机工作在这一层            │
├─────────────────────┼──────────────────────────────────────────────┤
│  1. 物理层           │  比特流在物理介质上的传输                       │
│  Physical           │  网线(电信号), 光纤(光信号), Wi-Fi(电磁波)       │
└─────────────────────┴──────────────────────────────────────────────┘
```
...
```
步骤1: 浏览器缓存检查
  → chrome://net-internals/#dns 可以看到浏览器级别的 DNS 缓存
  → 如果有缓存且未过期，直接返回IP，解析结束
...
步骤3: 本地 DNS 解析器（通常是路由器或 ISP 提供）
  → 检查路由器/ISP 的 DNS 缓存
  → 如果命中，返回IP，解析结束

步骤4: 递归查询（本地 DNS 解析器代替你完成以下过程）

  4a. 根域名服务器
      → 查询: "com 在哪？"
      → 回答: "这是 .com 的权威 DNS 服务器列表"
      → 全球 13 组根服务器（a.root-servers.net ~ m.root-servers.net）

  4b. .com 顶级域服务器
      → 查询: "your-domain.com 在哪？"
      → 回答: "这是 your-domain.com 的权威 DNS 服务器"

  4c. 你的域名的权威 DNS 服务器（通常是云服务商提供的）
      → 查询: "your-domain.com 的 A 记录是什么？"
      → 回答: "123.45.67.89"（你的云服务器公网IP）
      → 还会返回 TTL（Time To Live），告诉解析器这个结果可以缓存多久
```
...
ClientHello →
  ...
  - SNI: 要访问的域名（让服务器选择正确的证书）
                    ...
                    ← Certificate
                      服务器发送 SSL 证书链:
                      - 服务器证书（your-domain.com 的公钥）
                      - 中间 CA 证书
                      - （客户端用预装的根 CA 证书验证整条链）

                    ← ServerKeyExchange
                      密钥交换参数（如 ECDHE 的椭圆曲线公钥）
...
客户端                           服务器
  ...
  │                                 （状态变为 SYN_SENT）
  │                                │
  │←── SYN+ACK (seq=y, ack=x+1) ──│  ② 服务器：好的，我准备好了
  │                                 （状态变为 SYN_RCVD）
  │                                │
